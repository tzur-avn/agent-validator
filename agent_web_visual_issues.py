...
def capture_visual_node(state: AgentState):
    ...
    with sync_playwright() as p:
        ...
        context = browser.new_context(
            viewport={
                "width": state.get("viewport_width", 1920),
        ...
        for severity_name, issues_list in [
            ...
            if issues_list:
                ...
                for i, issue in enumerate(issues_list, 1):
                    ...
                    report += (
                        f"   Fix: {issue.get('recommendation', 'No recommendation')}\n"
                    )
...
if __name__ == "__main__":
    ...
    for config in test_configs:
        print(f"\n{'#'*80}")
        print(
            f"# Testing: {config['url']} at {config['viewport_width']}x{config['viewport_height']}"
        )
        print(f"{'#'*80}\n")
